...
impute_data <- function(X, scale = T, n.iter = 10, sel_method = c(1:16)) {
    ...
    # optional scaling
    if (scale == T) {X <- as.data.frame(scale(X))}
    ...
    list(random_replacement = random_imp_list, mean_imputation = mean_imp_list, median_imputation = median_imp_list,
        missMDA_reg_imputation = missMDA_reg_list, missMDA_EM_imputation = missMDA_EM_list, pcaMethods_PPCA_imputation = pcaMethods_PPCA_list,
        ...
        mice_mixed_imputation = mice_mixed_list, mi_Bayesian_imputation = mi_Bayesian_list, ameliaII_imputation = amelia_list,
