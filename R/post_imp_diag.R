...
#'
#' @return
#' \item{Densityplots}{List of density plots of all numeric variables. The density plots show the original values and the imputed values overlaid for each variables in the dataframe}
...
post_imp_diag <- function(X_orig, X_imp, scale=T, n.boot=1000) {
  ...
  if (factors_present == T) {X_orig_factor <- X_orig[!sapply(X_orig,is.numeric)]}
  ...
  if (factors_present == T) {
    for (i in 1:ncol(X_orig_factor)) {
      ...
      q <- ggplot(data = data.frame(x = vals[,1], y = vals[,2])
                  , aes(x = factor(y), fill = factor(x))) +
        geom_bar(position="fill") +
        ggtitle("Bar chart of original values and imputed values") +
        labs(x=colnames(X_orig_factor)[i]) +
  ...
  for (i in 1:ncol(X_orig_num)) {
    ...
    q <- ggplot(data = data.frame(x = vals[,2], y = vals[,1])
      ...
      labs(x=colnames(X_orig_num)[i], y="")
  ...
  for (i in 1:ncol(X_orig_num)) {
    for (j in 1:ncol(X_orig_num)) {
      for (b in 1:n.boot) {
        ...
        corrs[b] <- cor(X_orig_num[idx, ], use="pairwise.complete.obs", method="pearson")[i,j]
      }
  ...
  for (i in 1:ncol(X_imp_num)) {
    for (j in 1:ncol(X_imp_num)) {
      for (b in 1:n.boot) {
        ...
        corrs[b] <- cor(X_imp_num[idx, ], use="pairwise.complete.obs", method="pearson")[i,j]
      }
      meanmat[i,j] <- mean(corrs)
      locimat[i,j] <- quantile(corrs, c(0.025))
      hicimat[i,j] <- quantile(corrs, c(0.975))
  ...
  colnames(correlation_results) <- c("var1", "var2", "orig.cor", "orig.lo.ci",
  ...
  p <- ggplot(data=correlation_results, aes(x=orig.cor, y=imp.cor)) +
    ...
    geom_errorbarh(aes(xmin = orig.lo.ci,xmax = orig.hi.ci), alpha=0.1) +
    geom_abline(slope=1, intercept = 0, col = "blue", alpha=0.5) +
    geom_line(stat="smooth", method = "lm", col = "red", alpha = 0.5) +
    ggtitle("Scatter plot of correlation coefficients") +
