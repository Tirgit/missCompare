...
MAP <- function(X_hat, missfrac_per_var, assumed_pattern, window = 0.5) {
    ...
    if (length(assumed_pattern) != length(missfrac_per_var)) 
    ...
    to_NA <- sample(rownames(X_hat)[ind | NAs], missfrac_per_var[MAR_vars[length(MAR_vars)]] * 
        nrow(X_hat))
...
### LAB res <- MAP(yy$Simulated_matrix, y$Fraction_missingness_per_variable, assumed_pattern =
### c('MAR', 'MCAR', 'MCAR', 'MAR', 'MNAR', 'MCAR', 'MAR', 'MAR', 'MNAR', 'MNAR', 'MCAR', 'MAR',
### 'MNAR', 'MAR')) matrixplot(res$MAP_matrix, interactive = F, col= 'red')
