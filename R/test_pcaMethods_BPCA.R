...
#'
#' @description
#' \code{\link{test_pcaMethods_BPCA}} tests the imputation accuracy of the pcaMethods BPCA missing data imputation algorithm on matrices with various missing data patterns
#'
#' @details
#' This function tests the imputation accuracy of the pcaMethods BPCA missing data imputation algorithm by comparing the
...
test_pcaMethods_BPCA <- function(X_hat, list) {
    ...
    pcaMethods_BPCA_imp <- function(X) {
        ...
        if (ncomp$ncp > 0) 
            resBPCA <- pcaMethods::pca(X, method = "bpca", center = FALSE, nPcs = ncomp$ncp) else resBPCA <- pca(X, method = "bpca", center = FALSE, nPcs = 2)
    ...
    log_output <- utils::capture.output(results <- lapply(list, pcaMethods_BPCA_imp))
...
# y$Fraction_missingness_per_variable) test_pcaMethods_BPCA(X_hat =
