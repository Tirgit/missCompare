...
#' Kolmogorov–Smirnov test statistics (KS) between the imputed datapoints and the original datapoints (that were subsequently set to missing)
...
#' automatically detect whether there is a MAP matrix in the list and calculate metrics for all matrices provided in the list.
#'
...
#' \item{Plot_RMSE}{Faceted boxplot of RMSE values per missiness pattern and missing data imputation algorithm}
#' \item{Plot_MAE}{Faceted boxplot of MAE values per missiness pattern and missing data imputation algorithm}
#' \item{Plot_KS}{Faceted boxplot of KS values per missiness pattern and missing data imputation algorithm}
#'
#' @examples
#' \dontrun{
