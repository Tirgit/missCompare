...
#'
#' @description
#' \code{\link{test_mi}} tests the imputation accuracy of the mi missing data imputation algorithm on matrices with various missing data patterns
#'
#' @details
#' This function tests the imputation accuracy of the mi missing data imputation algorithm by comparing the
...
test_mi <- function(X_hat, list) {
    ...
    mi_imp <- function(X) {
        mi_data <- mi::mi(as.data.frame(X), n.chain = 1, n.iter = 100, verbose = FALSE)
    ...
    log_output <- utils::capture.output(results <- lapply(list, mi_imp))
...
# y$Fraction_missingness_per_variable) test_mi(X_hat = yy$Simulated_matrix, list =
# res)
