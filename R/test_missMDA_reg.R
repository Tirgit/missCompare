#' @title Testing the missMDA regularized missing data imputation algorithm
#'
#' @description
#' \code{\link{test_missMDA_reg}} tests the imputation accuracy of the missMDA regularized missing data imputation algorithm on matrices with various missing data patterns
#'
#' @details
#' This function tests the imputation accuracy of the missMDA regularized missing data imputation algorithm by comparing the
...
test_missMDA_reg <- function(X_hat, list) {
    ...
    missMDA_regularized_imp <- function(X) {
        ...
        res.imp <- missMDA::imputePCA(X, ncp = ncomp$ncp, method = "Regularized")
    ...
    log_output <- utils::capture.output(results <- lapply(list, missMDA_regularized_imp))
