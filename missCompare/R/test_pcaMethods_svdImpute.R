#' @title Testing the pcaMethods svdImpute missing data imputation algorithm
#'
#' @description
...
test_pcaMethods_svdImpute <- function(X_hat, list) {
  ...
  pcaMethods_svdImpute_imp <- function(X) {
    ...
    if (ncomp$ncp>0) ressvdImpute <- pcaMethods::pca(X, method="svdImpute", center=FALSE, nPcs=ncomp$ncp) else ressvdImpute <- pca(X, method="svdImpute", center=FALSE, nPcs=2)
